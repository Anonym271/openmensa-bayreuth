...
from . import feedubt
from .parserubt import known_canteens
...
def unknown_canteen(name):
    log.error('Unknown canteen requested: %s', name)
...
def get_feed_day(canteen_name: str):
    ...
    log.info('Got request: %s', request.url)
...
def get_feed_complete(canteen_name: str):
    ...
    try:
        ...
        log.debug('Collecting meals from %s to %s for mensa %s', start_day, stop_day, canteen_name)
        return feedubt.get_feed_range(start_day, stop_day, canteen_name).toXMLFeed()
    except Exception as err:
